...
export const POST: APIRoute = async ({ params, request }) => {
  try {
    ...
    const clarificationMessage = body.clarificationMessage || '';
    ...
    await writeClient.patch(id).set({ status: 'clarification-requested', clarificationMessage }).commit();

    await writeClient.create({
      _type: 'notification',
      recipient: { _type: 'reference', _ref: pto.employee._id },
      type: 'pto-clarification',
      message: `Your PTO request for ${pto.startDate} to ${pto.endDate} requires clarification: ${clarificationMessage}`,
      isRead: false,
