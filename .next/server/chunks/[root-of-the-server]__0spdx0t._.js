module.exports=[18520,e=>e.a(async(E,T)=>{try{let E=await e.y("@libsql/client-6da938047d5fc1cd");e.n(E),T()}catch(e){T(e)}},!0),62294,e=>e.a(async(E,T)=>{try{var t=e.i(18520),a=E([t]);[t]=a.then?(await a)():a;let o=(0,t.createClient)({url:process.env.TURSO_DATABASE_URL||"file:local.db",authToken:process.env.TURSO_AUTH_TOKEN});async function N(e,E=[]){let T=await o.execute({sql:e,args:E});return T.rows.map(e=>{let E={};return T.columns.forEach((T,t)=>{E[T]=e[t]}),E})}async function s(e,E=[]){let T=await o.execute({sql:e,args:E});return{lastInsertRowid:T.lastInsertRowid??0,changes:T.rowsAffected}}async function i(){await o.execute(`CREATE TABLE IF NOT EXISTS users (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
  )`);let E=await N("SELECT COUNT(*) as cnt FROM store_rules");if(E[0]?.cnt===0)for(let[e,E,T]of[["max_hours_per_week","40","Maximum hours per week before overtime"],["max_consecutive_days","6","Maximum consecutive work days"],["min_hours_between_shifts","8","Minimum hours between shifts (no close-open)"],["overtime_threshold","40","Weekly hours threshold for overtime"]])await s("INSERT INTO store_rules (rule_key, rule_value, description) VALUES (?, ?, ?)",[e,E,T]);let T=await N("SELECT COUNT(*) as cnt FROM departments");if(T[0]?.cnt===0)for(let[e,E]of[["Front End","#3b82f6"],["Produce","#22c55e"],["Meat","#ef4444"],["Grocery","#f59e0b"],["Deli","#8b5cf6"],["Bakery","#ec4899"]])await s("INSERT INTO departments (name, color) VALUES (?, ?)",[e,E]);let t=await N("SELECT COUNT(*) as cnt FROM bookmarks");if(t[0]?.cnt===0)for(let[e,E,T,t,a]of[["Schedule Manager","/manager/schedule","📅","#6366f1",1],["Employees","/manager/employees","👥","#22c55e",2],["PTO Queue","/manager/pto-queue","🗓️","#f59e0b",3],["Payroll","/manager/payroll","💰","#ec4899",4],["Settings","/manager/settings","⚙️","#64748b",5],["Market POS","https://app.marktpos.com/","🛒","#0ea5e9",6],["Revenue & Expenses","https://docs.google.com/spreadsheets/d/1Ixv27SxCO45EEAJwSy9OntV684Kqu18aXoruouHuRfs/edit","📊","#16a34a",7],["2026 Scheduling Sheet","https://docs.google.com/spreadsheets/d/1Zkw_Hn255ohWU6BUkz2EuF9sAzi10bOAfuxFKjxwwVA/edit","📆","#7c3aed",8],["Payroll Tracker","https://docs.google.com/spreadsheets/d/1mbfVPRybxb7N21uuxh-B_DmdJgFZf5PojDwKXtahFOc/edit","💵","#db2777",9],["Paychex Flex","https://login.flex.paychex.com/login_static/UsernameOnly.html","🏦","#b45309",10],["Gmail","https://mail.google.com/mail/u/0/#inbox","✉️","#dc2626",11],["Finance Dashboard","https://docs.google.com/spreadsheets/d/1D9G24Fvq_Z6L-P1LlEiXsUs1z8ebwttcOV1HEQfG8vc/edit","📈","#0891b2",12],["Google Drive","https://drive.google.com/drive/folders/1m2iyH8J2Ap2Ivn2fbEWAnNgR2k9fwgX0","📁","#ca8a04",13]])await s("INSERT INTO bookmarks (title, url, icon, color, sort_order) VALUES (?, ?, ?, ?, ?)",[e,E,T,t,a]);let a=await N("SELECT COUNT(*) as cnt FROM users WHERE role='admin'");if(a[0]?.cnt===0){let E=await e.A(74658),T=await E.default.hash("admin123",10);await s("INSERT INTO users (email, password_hash, role) VALUES (?, ?, 'admin')",["admin@store.com",T])}}e.s(["initDb",0,i,"query",0,N,"run",0,s]),T()}catch(e){T(e)}},!1),74658,e=>{e.v(e=>Promise.resolve().then(()=>e(43091)))}];

//# sourceMappingURL=%5Broot-of-the-server%5D__0spdx0t._.js.map